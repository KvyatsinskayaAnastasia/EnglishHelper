...
public class AddWordWithBotRequestTranslationActionHandler implements BotActionHandler {
    private final OllamaService ollamaService;

    @Override
    public BotAction getBotAction() {
        return BotAction.ADD_WORD_WITH_BOT_REQUEST_TRANSLATION;
    }

    @Override
    public SendMessage processAction(UserState userState, String message) {
        userState.getCurrentWordState().setOriginal(message);
        userState.setUserStatus(UserStatus.FILLING_TRANSLATION);
