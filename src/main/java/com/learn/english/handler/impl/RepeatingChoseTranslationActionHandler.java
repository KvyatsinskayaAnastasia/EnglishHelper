...
@Component
public class RepeatingChoseTranslationActionHandler extends RepeatingActionHandler {

    public RepeatingChoseTranslationActionHandler(WordService wordService, Random random) {
        super(wordService, random);
    }

    @Override
    public BotAction getBotAction() {
        return BotAction.REPEATING_CHOSE_TRANSLATION;
    }

    @Override
    protected UserStatus getUserStatus() {
        return UserStatus.REPEATING_CHOSE_TRANSLATION;
    }
    ...
    protected String getQuestionText(WordForRepeat currentWord) {
        return "Выберите корректный перевод слова: " + currentWord.getOriginal() + "\n";
    }

    @Override
    protected String getRetryText(WordForRepeat currentWord) {
        return "Попробуйте еще раз: " + currentWord.getOriginal() + "\n";
    }
