...
public class PlannedRepeatingGameHandler implements UserStatusHandler {
    private static final List<BotAction> ENABLED_CHOOSE_PLANNED_REPEATING_BOT_ACTIONS = List.of(
    ...
    public UserStatus getUserStatus() {
        return UserStatus.PLANNED_REPEATING_GAME;
    }

    @Override
    public void processAction(UserState userState, Update update) {
        var botAction = getBotAction(update);
        if (botAction != null && ENABLED_CHOOSE_PLANNED_REPEATING_BOT_ACTIONS.contains(botAction)
        ...
        userState.setLastBotCommandMessageId(telegramHelperService.sendMessage(sendWithActionButtons(userState.getUserId(),
                REPEATING_PLANNED.getAnswerMessage(), ENABLED_CHOOSE_PLANNED_REPEATING_BOT_ACTIONS, 2)));
        userStateService.saveUserState(userState);
    }
}
